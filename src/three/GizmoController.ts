...
export class GizmoController {
  ...
  private _updateIKInnerDrag(): void {
    ...
    if (innerDraggedObj.parent) {
      ...
      // Explicit updateMatrix() in case FBX bones have matrixAutoUpdate=false.
      innerDraggedObj.updateMatrix()
  ...
  private _solveFullBodyCascade(ds: DragState, target: THREE.Vector3): void {
    ...
    const extendedBoneMap: Partial<Record<BoneName, BoneName[]>> = {
      ...
      'hand.R': ['hips', 'spine', 'spine_upper', 'chest', 'shoulder.R', 'upper_arm.R', 'forearm.R', 'hand.R'],
      'head':   ['hips', 'spine', 'spine_upper', 'chest', 'neck', 'head'],
